...
def prepare_input_dataproto(tokenizer, config, validate):
    ...
    formatted_prompts = [tokenizer.apply_chat_template(conversation, tokenize=False, add_generation_prompt=True, enable_thinking=getattr(tokenizer, 'enable_thinking', True)) for conversation in preencode_prompts]
