...
if __name__ == "__main__":
    ...
    for data in all_data:
        ...
        data_by_source[data_source].append(data)

    def compute_metrics_for_data(data_list):
        """Compute all metrics for a list of data entries."""
        all_scores_f1 = []
        all_scores_em = []
        all_scores_mbe = []
        ...
        return {
            "count": len(data_list),
            "F1": sum(all_scores_f1) / len(all_scores_f1) if all_scores_f1 else 0,
            "EM": sum(all_scores_em) / len(all_scores_em) if all_scores_em else 0,
            "MBE": sum(all_scores_mbe) / len(all_scores_mbe) if all_scores_mbe else 0,
            "Compression Ratio": sum(all_compression_ratios) / len(all_compression_ratios) if all_compression_ratios else 0,
            "Peak Token Compression": sum(all_peak_token_compressions) / len(all_peak_token_compressions) if all_peak_token_compressions else 0,
            "Dependency Compression": sum(all_dependency_compressions) / len(all_dependency_compressions) if all_dependency_compressions else 0,
            "Peak Token Non-Compression": sum(all_peak_token_non_compressions) / len(all_peak_token_non_compressions) if all_peak_token_non_compressions else 0,
            "Dependency Non-Compression": sum(all_dependency_non_compressions) / len(all_dependency_non_compressions) if all_dependency_non_compressions else 0,
        }

    # Compute and print metrics for each data_source
    print("=" * 80)
    for data_source in sorted(data_by_source.keys()):
        metrics = compute_metrics_for_data(data_by_source[data_source])
        print(f"\nData Source: {data_source} (n={metrics['count']})")
        print("-" * 40)
    ...
    print("\n" + "=" * 80)
    print("OVERALL (all data sources combined)")
    print("=" * 80)
    overall_metrics = compute_metrics_for_data(all_data)
