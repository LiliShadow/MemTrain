...
class AsyncLLMGenerationManager:
    ...
    def concat_output(self, batch_list: list[dict]) -> DataProto:
        enable_thinking = getattr(self.tokenizer, 'enable_thinking', True)
        starting = self.tokenizer.apply_chat_template(STARTING_MSG, add_generation_prompt=True, enable_thinking=enable_thinking)
