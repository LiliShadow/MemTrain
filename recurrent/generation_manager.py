...
class LLMGenerationManager:
    ...
    def run_recall_loop(self, recall_messages, recall_meta_info, gen_batch_meta_info, timing_raw, recall_n):
        """Run memory recall generation after the main loop.

        Args:
            recall_messages: list of token-ID tensors from agent.recall()
            recall_meta_info: dict from agent.recall() with ground_truths, sample_indices etc.
            gen_batch_meta_info: meta_info from gen_batch (eos/pad token ids, do_sample etc.)
            timing_raw: timing dict
            recall_n: number of times to repeat each recall prompt (for GRPO-style grouping)

        Returns:
            recall_output: DataProto with generated recall predictions
        """
        if not recall_messages:
            return None

        with _timer('recall_prepare', timing_raw):
            # Repeat messages recall_n times for GRPO-style grouping
            repeated_messages = []
            for _ in range(recall_n):
                repeated_messages.extend(recall_messages)

            meta_info = {
                'input_pad_to': recall_meta_info['input_pad_to'],
                'pad_to': recall_meta_info['pad_to'],
                'generation_kwargs': recall_meta_info['generation_kwargs'],
            }
            meta_info.update(gen_batch_meta_info)

            pad_token_id = self.tokenizer.pad_token_id
            input_ids = pad_tensor_list_to_length(
                repeated_messages, pad_token_id=pad_token_id,
                max_length=meta_info['input_pad_to'], left_pad=True
            )
            attention_masks = create_attention_mask(input_ids, pad_token_id=pad_token_id)
            position_ids = create_position_ids(attention_masks)
            logger.info(f'recall padding done, {len(repeated_messages)} prompts')

        with _timer('recall_gen', timing_raw):
            recall_output = self.generate_with_graceful_padding(
                input_ids, attention_masks, position_ids, meta_info
            )
            logger.info('recall generation done')
