...
def chat_template(tokenizer, system=False) -> str:
    enable_thinking = getattr(tokenizer, 'enable_thinking', True)
    if system:
...
# spaCy NER entity extraction for memory recall
_spacy_nlp = None


def get_spacy_nlp():
    """Lazy-load spaCy model with only NER enabled."""
    global _spacy_nlp
...
def extract_entities_spacy(passage: str) -> list:
    """Extract named entities using spaCy NER. Returns deduplicated list preserving order."""
    nlp = get_spacy_nlp()
    doc = nlp(passage)
...
def check_occurrence(passage: str, word: str, max_occurrence: int = 5) -> bool:
    """Check if word occurs fewer than max_occurrence times using word boundary matching."""
    pattern = r'\b' + re.escape(word) + r'\b'
    return len(re.findall(pattern, passage)) < max_occurrence


def check_length_limit(text: str, max_words: int = 8) -> bool:
    """Check if text has fewer than max_words words."""
    return len(text.strip().split()) < max_words


def filter_entities(entities: list, passage: str, max_occurrence: int = 5, max_words: int = 8) -> list:
    """Filter entities by occurrence count and word length."""
    return [
        e for e in entities
        if e in passage and check_occurrence(passage, e, max_occurrence) and check_length_limit(e, max_words)
    ]


def get_random_entity_spacy(passage: str, max_occurrence: int = 5, max_words: int = 8) -> str:
    """Extract entities with spaCy, filter, and randomly select one. Returns empty string if none found."""
