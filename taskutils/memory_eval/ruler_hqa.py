...
def get_pred(data, args, out_file):
    ...
    if args.save_trajectory:
        traj_path = out_file.replace('.jsonl', '_trajectory.jsonl')
        traj_file = open(traj_path, 'w' if args.force else 'a', encoding='utf-8')
    for i, (output, item) in enumerate(zip(outputs, data)):
        ...
        if trajectory is not None and traj_file is not None:
            traj_item = {
                '_id': item['_id'],
                'input': item['input'],
            ...
            traj_file.write(json.dumps(traj_item, ensure_ascii=False) + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tokenizer", "-t", type=str, default="models/Qwen/Qwen2.5-7B-Instruct")
