...
MODELROOT = os.getenv("MODELROOT", "/models")
...
class Config:
    ...
    def serve(self, wait=True):
        ...
        if wait:
            ...
            env = os.environ.copy()
            serve_p = subprocess.Popen(cmd.split(), preexec_fn=os.setsid, env=env)
    ...
    def run(self, tests, serve=True, force=False, save_trajectory=False):
...
memagent_4B = Config(
    name="Memagent-4B-ins",
    ckpt="checkpoints/memagent/Qwen3-4B-Instruct-2507.2/global_step_500/huggingface",
    tp=1,
    method="recurrent",
    concur=256,
    env=ENV(RECURRENT_MAX_CONTEXT_LEN=100000000000, RECURRENT_CHUNK_SIZE=5000, RECURRENT_MAX_NEW=1024),
...
def run_ruler_hqa(save_trajectory=False):
    for c in CONFIGS:
        task = RULER_HQA_TESTS
        # if c.name.startswith("MemoryAgent"):
        #     task += RULER_HQA_TESTS_OVER_1M
        c.run(task, serve=True, force=False, save_trajectory=save_trajectory)


def run_ood_tasks(save_trajectory=False):
    for c in CONFIGS:
        subset = [
            "niah_single_1",
            "niah_single_2",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_trajectory', action='store_true',
                        help='Save intermediate chunk iterations to separate trajectory file')
    args = parser.parse_args()

    print(f"{SERVE_PORT=}, {DASH_PORT=}, {MODELROOT=}")
    run_ruler_hqa(save_trajectory=args.save_trajectory)
    run_ood_tasks()
