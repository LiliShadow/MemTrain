...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        enable_thinking = self.config.model.get("enable_thinking", True) if hasattr(self.config, 'model') else True
        self.tokenizer = hf_tokenizer(local_path, trust_remote_code=trust_remote_code, enable_thinking=enable_thinking)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        enable_thinking = config.model.get("enable_thinking", True)
        self.tokenizer = hf_tokenizer(tokenizer_path, trust_remote_code=config.model.get("trust_remote_code", False), enable_thinking=enable_thinking)
...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        if self.config.model.input_tokenizer is None:
            ...
            enable_thinking = config.model.get("enable_thinking", True)
            self.input_tokenizer = hf_tokenizer(input_tokenizer_local_path, trust_remote_code=config.model.get("trust_remote_code", False), enable_thinking=enable_thinking)
            self.tokenizer = hf_tokenizer(local_path, trust_remote_code=config.model.get("trust_remote_code", False), enable_thinking=enable_thinking)
