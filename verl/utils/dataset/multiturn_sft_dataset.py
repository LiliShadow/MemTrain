...
class MultiTurnSFTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        full_tokens = tokenizer.apply_chat_template(messages, tokenize=True, return_tensors="pt", add_generation_prompt=False, enable_thinking=enable_thinking)
        ...
        for i, msg in enumerate(messages):
            ...
            prefix_tokens = tokenizer.apply_chat_template(prefix_messages, tokenize=True, return_tensors="pt", add_generation_prompt=False, enable_thinking=enable_thinking)

            # Get tokens for messages up to previous point
            prev_tokens = tokenizer.apply_chat_template(messages[:i], tokenize=True, return_tensors="pt", add_generation_prompt=False, enable_thinking=enable_thinking) if i > 0 else None
