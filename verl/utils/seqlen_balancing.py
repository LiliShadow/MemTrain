...
def rearrange_micro_batches(batch: TensorDict, max_token_len, dp_group=None):
    ...
    assert num_micro_batches <= len(seq_len_effective), f"num_micro_batches must be less than or equal to the batch size. Got {num_micro_batches=} and {len(seq_len_effective)=}"
