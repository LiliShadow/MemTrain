...
def compute_data_metrics(batch: DataProto, use_critic: bool = True, compute_reward_metrics: bool = True) -> Dict[str, Any]:
    ...
    # Only compute reward/score/pass@n metrics if requested (to avoid overwriting pre-filtering metrics)
    if compute_reward_metrics:
        metrics.update({
            # passrate
            'critic/pass@n':
